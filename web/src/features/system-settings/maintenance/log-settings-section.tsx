...
const logSettingsSchema = z.object({
  ...
  ConversationRecordEnabled: z.boolean(),
})
...
type LogSettingsSectionProps = {
  ...
  defaultConversationRecordEnabled?: boolean
}
...
export function LogSettingsSection({
  defaultEnabled,
  defaultConversationRecordEnabled = false,
  ...
  const form = useForm<LogSettingsFormValues>({
    ...
    defaultValues: {
      ...
      ConversationRecordEnabled: defaultConversationRecordEnabled,
    },
  })
  ...
  const [conversationPurgeDate, setConversationPurgeDate] = useState<
    Date | undefined
  >(() => getDateDaysAgo(30))
  const [isCleaningConversation, setIsCleaningConversation] = useState(false)
  ...
  }, [defaultEnabled, defaultConversationRecordEnabled, form])
  ...
  const onSubmit = async (values: LogSettingsFormValues) => {
    ...
    if (values.ConversationRecordEnabled !== defaultConversationRecordEnabled) {
      updates.push(
        updateOption.mutateAsync({
          key: 'ConversationRecordEnabled',
          value: values.ConversationRecordEnabled,
        })
      )
    }
  ...
  const conversationPurgeTimestamp = useMemo(() => {
    if (!conversationPurgeDate) return null
    return Math.floor(conversationPurgeDate.getTime() / 1000)
  }, [conversationPurgeDate])

  const formattedConversationPurgeDate = useMemo(() => {
    if (!conversationPurgeDate) return ''
    return formatTimestampToDate(conversationPurgeDate.getTime(), 'milliseconds')
  }, [conversationPurgeDate])

  const handleCleanConversationRecords = async () => {
    if (!conversationPurgeTimestamp) {
      toast.error(t('Select a timestamp before clearing conversation records.'))
      return
    }
    setIsCleaningConversation(true)
    try {
      const res = await api.delete(
        `/api/log/conversation/?target_timestamp=${conversationPurgeTimestamp}`
      )
      if (!res.data.success) {
        throw new Error(
          res.data.message || t('Failed to clean conversation records')
        )
      }
      const count = res.data.data ?? 0
      toast.success(
        count > 0
          ? t('{{count}} conversation records removed.', { count })
          : t('No conversation records matched the selected time.')
      )
      ...
      const message =
        error instanceof Error
          ? error.message
          : t('Failed to clean conversation records')
      toast.error(message)
    } finally {
      setIsCleaningConversation(false)
    }
  }
    ...
    <SettingsSection title={t('Log Maintenance')}>
      <Form {...form}>
        <SettingsForm onSubmit={form.handleSubmit(onSubmit)}>
          ...
          <FormField
            control={form.control}
            name='ConversationRecordEnabled'
            render={({ field }) => (
              <SettingsSwitchItem>
                <SettingsSwitchContent>
                  <FormLabel>{t('Enable conversation recording')}</FormLabel>
                  <FormDescription>
                    {t(
                      'Store raw request bodies for consume logs so admins can inspect prompts. Increases storage usage.'
                    )}
                  </FormDescription>
                </SettingsSwitchContent>
                <FormControl>
          ...
          <SettingsControlGroup className='space-y-3'>
            <div>
              ...
              <p className='text-muted-foreground text-sm'>
                {t(
                  'Remove conversation records created before the selected timestamp.'
                )}
              </p>
            </div>
            <DateTimePicker
              value={conversationPurgeDate}
              onChange={setConversationPurgeDate}
            />
            <div className='flex flex-wrap gap-3'>
              {quickSelectOptions.map((option) => (
                <Button
                  key={`conv-${option.label}`}
                  type='button'
                  variant='outline'
                  onClick={() => setConversationPurgeDate(option.getValue())}
                >
                  {t(option.label)}
                </Button>
              ))}
              ...
              >
                ...
                <AlertDialogContent>
                  <AlertDialogHeader>
                    <AlertDialogTitle>
                      {t('Confirm clear conversation records')}
                    </AlertDialogTitle>
                    <AlertDialogDescription>
                      {t(
                        'This will permanently delete all conversation records before {{date}}.',
                        { date: formattedConversationPurgeDate }
                      )}
                    </AlertDialogDescription>
                  </AlertDialogHeader>
                  <AlertDialogFooter>
                    ...
                    <AlertDialogAction
                      onClick={handleCleanConversationRecords}
                      disabled={isCleaningConversation}
                    >
                      {t('Confirm')}
                    </AlertDialogAction>
                  </AlertDialogFooter>
