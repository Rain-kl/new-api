...
func (a *Adaptor) ConvertOpenAIRequest(c *gin.Context, info *relaycommon.RelayInfo, request *dto.GeneralOpenAIRequest) (any, error) {
	...
	if info.ChannelType != constant.ChannelTypeOpenRouter && renderReasoning {
		...
		// When the channel has already set the reasoning effort, skip suffix-intent
		// merging to avoid a conflict error — treat the suffix as a model-name-only hint.
...
func (a *Adaptor) ConvertOpenAIResponsesRequest(c *gin.Context, info *relaycommon.RelayInfo, request dto.OpenAIResponsesRequest) (any, error) {
	...
	if (info == nil || info.GetChannelType() != constant.ChannelTypeOpenRouter) && !crossProtocol && effort == "" && originEffort == "" && request.ReasoningConversion == nil && info.ReasoningState() == nil {
		if info != nil {
			rawEffort := ""
