...
func HandleGroupRatio(ctx *gin.Context, relayInfo *relaycommon.RelayInfo) hosttypes.GroupRatioInfo {
	...
	// channel ratio from the selected channel (set by SetupContextForSelectedChannel)
	if v, ok := common.GetContextKey(ctx, constant.ContextKeyChannelRatio); ok {
		if f, ok := v.(float64); ok {
			groupRatioInfo.ChannelRatio = f
		}
	}
...
func ModelPriceHelper(c *gin.Context, info *relaycommon.RelayInfo, promptTokens int, meta *types.TokenCountMeta) (hosttypes.PriceData, error) {
	...
	// The billing model is the requested model, unless that model has no fee
	...
	if !operation_setting.GetQuotaSetting().EnableFreeModelPreConsume {
		...
		if groupRatioInfo.GroupRatio == 0 || groupRatioInfo.ChannelRatio == 0 {
			preConsumedQuota = 0
			freeModel = true
		} else if usePrice {
			if modelPrice == 0 {
	...
	if usePrice {
		...
		quotaToPreConsume := priceData.ApplyOtherRatiosToFloat(modelPrice * common.QuotaPerUnit * groupRatioInfo.GroupRatio * groupRatioInfo.ChannelRatio)
...
func ModelPriceHelperPerCall(c *gin.Context, info *relaycommon.RelayInfo) (hosttypes.PriceData, error) {
	...
	// Same mapping fallback as ModelPriceHelper: bill by the mapped target model
	// when the requested model itself has no fee configured.
	...
	if usePrice {
		var err error
		quota, err = common.QuotaFromFloatStrict(modelPrice * common.QuotaPerUnit * groupRatioInfo.GroupRatio * groupRatioInfo.ChannelRatio)
		if err != nil {
			return hosttypes.PriceData{}, err
		}
		if !operation_setting.GetQuotaSetting().EnableFreeModelPreConsume {
			if groupRatioInfo.GroupRatio == 0 || groupRatioInfo.ChannelRatio == 0 || modelPrice == 0 {
				quota = 0
				freeModel = true
			}
		}
		...
		// 按量计费：以模型倍率的一半作为预扣额度
		var err error
		quota, err = common.QuotaFromFloatStrict(modelRatio / 2 * common.QuotaPerUnit * groupRatioInfo.GroupRatio * groupRatioInfo.ChannelRatio)
		if err != nil {
			return hosttypes.PriceData{}, err
		}
		modelPrice = -1
		if !operation_setting.GetQuotaSetting().EnableFreeModelPreConsume {
			if groupRatioInfo.GroupRatio == 0 || groupRatioInfo.ChannelRatio == 0 || modelRatio == 0 {
				quota = 0
				freeModel = true
			}
		}
...
// for the request. It is the origin model unless that model has no fee configured
// (no price, no ratio, no tiered expr), the user has not opted into unset-ratio
// models, and the selected channel's model_mapping points the model at a target
...
// channelMappedModel follows the selected channel's model_mapping chain (with
...
func channelMappedModel(c *gin.Context, start string) (string, bool) {
	...
	for {
		...
		if visited[next] {
			if next == current {
				break // self-map terminates the chain
			}
			return "", false // genuine cycle
		}
		visited[next] = true
		current = next
	}
...
func modelPriceHelperTiered(c *gin.Context, info *relaycommon.RelayInfo, billingModelName string, promptTokens int, meta *types.TokenCountMeta, groupRatioInfo hosttypes.GroupRatioInfo) (hosttypes.PriceData, error) {
	...
	if estimatedCompletionTokens == 0 && groupRatioInfo.GroupRatio != 0 && groupRatioInfo.ChannelRatio != 0 {
	...
	preConsumedQuota, err := billingexpr.QuotaRoundStrict(quotaBeforeGroup * groupRatioInfo.GroupRatio * groupRatioInfo.ChannelRatio)
	if err != nil {
		return hosttypes.PriceData{}, err
	}
	...
	snapshot := &billingexpr.BillingSnapshot{
		...
		ChannelRatio:              &groupRatioInfo.ChannelRatio,
