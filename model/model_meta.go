...
func (mi *Model) Update() error {
	return metadataTransaction(func(tx *gorm.DB) error {
		...
		return tx.Model(&Model{}).Where("id = ?", mi.Id).
			Select("model_name", "description", "icon", "tags", "vendor_id", "endpoints", "status", "sync_official", "context_window", "max_output_tokens", "name_rule", "updated_time").Updates(mi).Error
	})
}
